	global baseline		$baseline_ctrls
	global embank		ero2 ero3 meghna
	global credit		brac_any
	global matlab		pour
	global migstatus	curr_mig_od curr_mig_urban
	global none		
	global controls		embank matlab credit migstatus none
	...
	* Get estimates
	foreach ctrl in baseline $controls {
		foreach index of global indexes {
			...
			qui reg `index' $treat ${`ctrl'} i.$birthyr ${`wt'} if $group==61, cluster($cluster)
				est sto `index'
	...
	esttab using "si_table7.csv" ///
			, ci(%4.2f) b(%4.2f) s(N) l r noobs compress ///
			br plain star(+ 0.10 * 0.05 ** 0.01)  ///
			mtitle("Baseline" "Embankment" "Urban" "Microcredit" "Own Migration" "No Controls") 