	global	FILENAME		"si_table1"
		cap erase "${FILENAME}.xls"
	cap drop control
	gen control = $treat == 0 if !missing($treat)

	cap postclose balance  
	postfile balance str40(outcome) float(meanT seT Ntreat meanC seC Ncontrol diffmeans difftstat diffsd_all)	///
		using "${FILENAME}", replace 
		
	foreach var of global baseline_ctrls { 
		...
		*Output the means, differences in means, std. errs, t-stat, & Ns. 
			post balance ("`: var lab `var''")  (meanT) (seT) (ntreat) (meanC) (seC) (ncontrol) (diffmean) (difftstat) (diffsd_all) 
			scalar drop _all
	  } 
	postclose  balance 
	preserve
